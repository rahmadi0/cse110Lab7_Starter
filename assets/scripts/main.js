    ...
    if ('serviceWorker' in navigator) {
        ...
        window.addEventListener('load', () => {
            navigator.serviceWorker
                .register('./sw.js')
                .then(registration => console.log('Service Worker successful'))
...
async function getRecipes() {
    ...
    var recipesItem = localStorage.getItem('recipes');
    if (recipesItem != null) {
        return JSON.parse(recipesItem);
    }
    ...
    const promiseFunction = new Promise(callbackFuncion);

    async function callbackFuncion(resolve, reject) {
        ...
        for (let i = 0; i < RECIPE_URLS.length; i++) {
            ...
            try {
                ...
                let retrirveJson = await response.json();
                recipesHoldItem.push(retrirveJson);

                if (recipesHoldItem.length == RECIPE_URLS.length) {
                    saveRecipesToStorage(recipesHoldItem);
                    resolve(recipesHoldItem);
                }
            } catch (e) {
                console.error(e);
                reject(e);
